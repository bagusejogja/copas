  ...
  return (
    <div>
      ...
      >
        ...
        {/* Icon */}
        <span className={`text-lg shrink-0 ${isGroup ? '' : ''}`}>
          {isGroup ? '📁' : (level === 0 ? '🏛️' : '🏢')}
        </span>
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button onClick={() => onAddChild(node.id)} className="text-[10px] px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg font-bold hover:bg-emerald-100">+ Anak</button>
          <button onClick={() => onEdit(node)} className="text-[10px] px-2 py-1 bg-blue-50 text-blue-600 rounded-lg font-bold hover:bg-blue-100">✎ Edit</button>
        </div>
      </div>
...
export default function ManajemenUnitPage() {
  ...
  const expandAll = () => {
    setExpandedIds(new Set(flatUnits.map(u => u.id)));
  };
  const collapseAll = () => {
  ...
  const openAddChild = (parentId: number) => {
    setEditId(null);
    setForm({ ...EMPTY_FORM, parent_unit_id: String(parentId) });
    setShowForm(true);
  };
  ...
  const openEdit = (unit: UnitNode) => {
    ...
    setForm({
      ...
      nama_unit_pendek: unit.nama_unit_pendek || '',
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        nama_unit_pendek: form.nama_unit_pendek || null,
        pemerhati: form.pemerhati || null,
        tipe: form.tipe,
        parent_unit_id: form.parent_unit_id ? Number(form.parent_unit_id) : null
      };
  ...
  const parentName = form.parent_unit_id
    ? flatUnits.find(u => u.id === Number(form.parent_unit_id))?.nama_unit || '-'
    : '(Root / Tidak Ada Induk)';
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6">
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">Mulai dengan menambahkan unit root (PDM).</p>
          </div>
        ) : (
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  type="text" placeholder="Misal: SDM Suronatan"
                  className="w-full border-gray-200 rounded-xl p-3 text-sm focus:ring-muh-green"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    value={form.nama_unit_pendek}
                    onChange={e => setForm({ ...form, nama_unit_pendek: e.target.value })}
                    type="text" placeholder="SDM-SR"
                    className="w-full border-gray-200 rounded-xl p-3 text-sm"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full border-gray-200 rounded-xl p-3 text-sm font-bold"
                  >
                ...
                >
                  ...
                  {flatUnits.filter(u => u.id !== editId).map(u => (
                    <option key={u.id} value={u.id}>
                      {u.parent_unit?.nama_unit ? `${u.parent_unit.nama_unit} → ` : ''}{u.nama_unit} [{u.tipe}]
                    </option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <input
                  value={form.pemerhati}
                  onChange={e => setForm({ ...form, pemerhati: e.target.value })}
                  type="text" placeholder="Nama Pemerhati (Opsional)"
                  className="w-full border-gray-200 rounded-xl p-3 text-sm"
                />
              </div>
            </div>
